...
%macro mvmodels(
                    ...
                    %do c = 1 %to &_columns;
                        %do g = 1 %to &_groups;
                            %if %sysevalf(%superq(model_title&i)^=,boolean) or
                                ...
                                (&nmodels>1 and %sysevalf(%superq(by&i)=,boolean)) %then %do;
                            set modelnum=&i,
                                subtitle=
                                case
    ...
    %if (&max_cat_covs>0 or &max_cont_covs>0) and %sysevalf(%qupcase(&method)=LOGISTIC,boolean) and 
        ...
        %if &min_strata = 0 %then %do;    
            proc logistic data=_tempdsn;
                ...
                ods output parameterestimates=_parm /**Parameter estimates and wald p-values**/
                       ...
                       %if &max_cat_covs>0 %then %do; modelanova=_t3 /**Type 3 p-values**/ %end;
        ...
        %if &nstrata >0 %then %do;        
            proc logistic data=_tempdsn;
                ...
                %if &min_strata = 0 %then %do;    
                    ods output parameterestimates=_parm_s /**Parameter estimates and wald p-values**/
                       ...
                       %if &max_cat_covs>0 %then %do; modelanova=_t3_s /**Type 3 p-values**/ %end;
            ...
            run;
            %if %sysfunc(exist(_t3))=0 %then %do;            
                %if %sysfunc(find(%superq(pval_type3),1)) and &min_strata>0 %then %put WARNING: LOGISTIC procedure requires at least one categorical covariate to produce type-3 p-values;
                proc sql;
                    create table _t3 
                        (model num,effect char(32),probchisq num);
                quit;
            %end;
            %if %sysfunc(exist(_t3_s))=0 %then %do;
                proc sql;
                    create table _t3_s 
                        (model num,effect char(32),probchisq num);
                quit;
