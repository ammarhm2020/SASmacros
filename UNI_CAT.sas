...
Last Update Date/Person/Contact: Oct. 9, 2013/Dana Nickleach/dnickle@emory.edu
Current Version: V28

Purpose:  To conduct a univariate analysis for a categorical outcome with all other covariates
in the data individually.  For categorical covariates, the contingency table along with the 
Chi-square test (parametric p-value) or Fisher?s exact test (non-parametric p-value) will be 
produced. For numerical covariates, the sample size, mean and median along with ANOVA test 
(parametric p-value) or Kruskal-Wallis test (non-parametric p-value) will be carried out.
...
decide which statistics (parametric or non-parametric p-value) is more appropriate to the data.

Parameters: 

DATASET        The name of the data set to be analyzed.
OUTCOME        List of categorical outcome variables, separated by empty space.  Each variable 
               name must not be more than 30 characters long.                                           
CLIST          List of categorical variables, separated by empty space.
NLIST          List of numerical variables, separated by empty space.
NONPAR         Value of F, T, or A to indicate whether to conduct non-parametric tests. For 
               example, in cases of a large sample size, parametric tests will be sufficient.  If 
               the value is T then both parametric and non-parametric tests will be conducted.  If 
               the value is F then only parametric tests will be conducted.  A value of A means 
               that for categorical variables, the appropriate test statistics non-parametric or 
               parametric will be automatically chosen for you based on whether the chi-square 
               test is invalid, but for numerical covariates, only the parametric test will be 
               calculated.  Option A is only available for SAS V9.3 or later.  The default value 
               is F.
SPREAD         Set to T to also report standard deviation, min, and max for numerical variables
               (optional).  The default value is F. 
BY             A separate analysis will be conducted for each value of the variable specified here
               (optional).  Note that the distinct variable values not formats will be used.  
MHC            Value of F or T to indicate whether to report p-values from Mantel-Haenszel 
               chi-square tests instead of Pearson chi-square test p-values (optional).  If the 
               value is T then Mantel-Haenszel chi-square will be used.  The default value is F.  
...
ROWPERCENT     Value of F or T to indicate whether the row or column percentage will be shown for 
               the contingency table.  The default value is T.
ORIENTATION    Value of PORTRAIT or LANDSCAPE to indicate the paper layout of the report
WEIGHT         Variable to use as weight in a weights statement (optional). Weights will need to 
               be normalized in the data preparation stage, but not in this macro. If the weight 
               is normalized properly, the reported N will be the sum of the weights. This option 
               will not work with NONPAR = T or A. Leave blank if not weighting.
...
PAR		Parametic.
MEANS		Request means and Standard deviation.
MEDIANS		Request medians and Qrange.

***********************************************************************************************
For more details, please see the related documentation
**********************************************************************************************/

%MACRO UNI_CAT(DATASET=, outcome=, CLIST=, NLIST=, NONPAR=F, PAR=F, MEANS=T, MEDIANS=T, SPREAD=F, BY=, DOC=T, OUTPATH=,
     FNAME =, ROWPERCENT = T, ORIENTATION = PORTRAIT , WEIGHT=, MHC = F, DEBUG=F); 

   /* Make sure these vars are local */
   %local OUTVAR NUM_OUT i STATL CVAR TVAR j __MACRO_ERR WORK_SETS COLABEL m TABLESUM 
     NONP_CTEST n COL OUT_CNT NVAR CTEST VV cvar_cnt check;
   ...
   %let par = %UPCASE(&par);
   %let medians= %UPCASE(&medians);
   %let means = %UPCASE(&means);
       ...
       %DO %UNTIL (&outvar = %STR());
         ...
         %DO %UNTIL (&nvar =  ); 
            ...
            RUN;
             ...
             PROC MEANS DATA=_data noprint p25 p75 qrange; 
               var &NVAR; 
               class &by &outvar;  
                 output out=_summary mean=mean2 median=median2 std=std2 min=min2 max=max2 p25=p252 p75=p752 qrange=qrange2
                 %if &weight ~= %STR() %then %do; 
                  SUMWGT=n2 
               %end;
               %else %do; n=n2 %end;; 
   		...
   		Qrange= LEFT(PUT(round(qrange2,.01),best12.));

p25=LEFT(PUT(round(p252,.01),best12.));
p75= LEFT(PUT(round(p752,.01),best12.));
            ...
            run;
             proc transpose data=_summary out=_summaryt; 
               var N mean median  %if &spread = T %then min max std p25 p75 qrange;; 
               id outcat;
                  %if &by ~= %STR() %then %do; BY &by; %end;
 ...
 %if &MEANS = F %then %do;
                        PROC SQL noprint;
                            delete from _report
                            where Statistics= 'Mean' or Statistics= 'Std Dev';
                       QUIT;
                      %end;

 %if &MEDIANS = F %then %do;
                        PROC SQL noprint;
                            delete from _report
                            where Statistics= 'Median' or Statistics= 'Qrange';
                       QUIT;
                      %end;
   ...
   PROC REPORT DATA=_report HEADLINE CENTER STYLE(REPORT)={JUST=CENTER} SPLIT='~' nowd 
     SPANROWS LS=256;
      COLUMNS &by _ordertyp _order Covariate  statistics level 
          %do i = 1 %to &num_out; 
                  ("&&outcomevv&i" '_____________________________'( &&categories&i))  
                   %if &PAR = T %then %do; par_p&i %end;
