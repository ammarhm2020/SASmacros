...
*                                                                                    ;
* The numargs macro was developed by Carrie Wager,Programmer,Channing Laboratory 1990;
...
proc format;
 value levelfmt 0="&label0" 1="&label1" 2="&label2" 3="&label3" 4="&label4" 5="&label5"
 6="&label6" 7="&label7" 8="&label8" 9="&label9" 10="&label10" 11="&label11" 12="&label12"
 13="&label13" 14="&label14" 15="&label15";
...
proc sql noprint;
 select count(unique(&exposure)) into :count from table1dat where &exposure >= 0;
 select min(&exposure) into :minexp from table1dat where &exposure >= 0;
 select max(&exposure) into :maxexp from table1dat where &exposure >= 0;
 select unique(&exposure) into :expval separated by ' ' from table1dat where &exposure >= 0;
 select count(*) into :countobs from table1dat where &exposure >= 0;
quit;
...
%let maxexp = %sysfunc(compress(&maxexp));
   ...
   %if "%upcase(&multn)" = "T" %then %do i = &minexp %to &maxexp;
     %let exp_n&i =;
   proc sql;    
    select %bquote(count(unique(&id))) into :exp_n&i from table1dat where &exposure=&i; 
   quit;
  %end;
...
data tab;
set tab;
%if &count > 1 %then %do; _nall_ = sum(of n&minexp-n&maxexp); %end;
%if &count = 1 %then %do; _nall_ = n&minexp; %end;
run;
...
%let mis=;
     ...
     /****  Calculate missing percent  *******/
      %if "%upcase(&miscol)" = "T" %then %do;
 	   proc sql noprint;
	    select (1-(_nall_/&countobs))*100 into :mis from tab having upcase(varname) = "%upcase(&var)";
	   quit;
        ...
        %if "&polycat" ne "" %then %do p = 1 %to &polyord;
                    %if %upcase(&var) = %upcase(_tmp&p) %then %do;
                       proc sql noprint;
                      	select (1-(_nall_/&countobs))*100 into :mis from tab having upcase(varname) = "%upcase(&varpol)";
                      quit;
...
%then %do;
 ...
 select p25&i into :brow&i from tab where upcase(varname)="%upcase(&var)";
 select p75&i into :crow&i from tab where upcase(varname)="%upcase(&var)";
...
%if &count > 5 or %eval(&count+&dec > 5) %then %do; %let landscape = T; %end;

%let nadjfn=;
%if "%upcase(&fn_noadj)" ne "" %then %do i = 1 %to %numargs(&fn_noadj); 
  %if "%upcase(&var)" eq "%upcase(%scan(&fn_noadj, &i, %str( )))" %then %do; %let nadjfn = %str(^{super *});
 %end;
%end;
...
%do j = &minexp %to &maxexp; /* 1 %to &count;*/
 ...
 value = cats("&var_lbl", "&nadjfn", "^{super &fn}"); %end;

 %if "%upcase(&var)" eq "%upcase(&catv)" %then %do;
     %if "%upcase(&pctn)" eq "N" %then %do;
         value = cats("&var_lbl", "&nadjfn", "^{super &fn}",', n'); %end;
...
%if "%upcase(&multn)" ne "T" %then %do; 
  %if "%upcase(&noexp)" ne "T" %then %do; label var&j = %sysfunc(cats("%unquote(&&lbl&j)", ' (n=', "%unquote(&&n&j)", ')' )); %end;
  %if "%upcase(&noexp)" eq "T" %then %do; label var&j = %sysfunc(cats(' (n=', "%unquote(&&n&j)", ')' )); %end;
%end;
%if "%upcase(&multn)" eq "T" %then %do; 
  %if "%upcase(&noexp)" ne "T" %then %do; label var&j = %sysfunc(cats("%unquote(&&lbl&j)", ' (n\/N = ', "%unquote(&&exp_n&j)",' \/ ', "%unquote(&&n&j)", ')' )); %end;
  %if "%upcase(&noexp)" eq "T" %then %do; label var&j = %sysfunc(cats(' (n\/N = ', "%unquote(&&exp_n&j)",' \/ ', "%unquote(&&n&j)", ')' )); 
%end;
%end;
...
drop tmp_n1 tmp_n2 tmp_c1 tmp_c2 tmp_n1b tmp_n2b tmp_n3 tmp_n4 tmp_n5 tmp_c3 tmp_c4 tmp_c5 tmp_n3b tmp_n4b tmp_n5b;
...
ods listing close;
ods rtf file="./&file..doc" style=Custom; 
ods escapechar='^'; 
...
%if "%upcase(&sep)" = "PM" %then %do; 
  %if "%upcase(&ageadj)" ne "F" %then %do;  ODS rtf text = "^S={Font=('Times Roman',12pt)}Values are means %sysfunc(BYTE(177)) SD or 
  percentages and are standardized to the age distribution of the study population."; %end;

  %if "%upcase(&ageadj)" eq "F" %then %do;  ODS rtf text = "^S={Font=('Times Roman',12pt)}Values are means %sysfunc(BYTE(177)) SD 
  or percentages."; %end;
%end;


%if "%upcase(&sep)" = "PAR" %then %do; 
  %if "%upcase(&ageadj)" ne "F" %then %do;  ODS rtf text = "^S={Font=('Times Roman',12pt)}Values are means(SD) or medians(Q25, Q75) for continuous variables; percentages or ns or both for categorical variables, and are 
standardized to the age distribution of the study population."; %end;

  %if "%upcase(&ageadj)" eq "F" %then %do;  ODS rtf text = "^S={Font=('Times Roman',12pt)}Values are means(SD) or medians(Q25, Q75) for continuous variables; percentages or ns or both for categorical variables."; %end;
%end;


%if "&poly" ne "" or "&polycat" ne "" %then %do; 
  ODS rtf text = "^S={Font=('Times Roman',12pt)} Values of polytomous variables may not sum to 100% due to rounding"; %end;
...
ODS rtf text = "^S={Font=('Times Roman',12pt)} ^{super *} Value is not age adjusted"; %end;
...
%macro table1(data=,
	      ...
	      covar=,
              ...
              explab=,
              label0=level 0,
              label1=level 1, label2=level 2, label3=level 3,
              label4=level 4, label5=level 5,
              label6=level 6, label7=level 7, label8=level 8, 
              label9=level 9, label10=level 10,
              label11=level 11, label12=level 12, label13=level 13, 
              label14=level 14, label15=level 15,
              notes=nonotes, 
...
proc means noprint data=_tmp_;  var &exposure;
   output  out=_minmax_  min=minexp  max=maxexp;
   run;
data _minmax_;  set _minmax_;
   call symput('_minexp_', trim(left(minexp)));  call symput('_maxexp_', trim(left(maxexp)));
...
proc freq data=_tmp_; 
   ...
   run;
data mainfreq;  set mainfreq;  pctov=percent; 
   keep pctov &agegroup;
   run;
...
%if %length(&extstand) ne 0 %then %do;
   data mainfreq;  set &extstand;  keep pctov &agegroup;  run;
%end;
   
proc sort data=_tmp_;  by &exposure;  run;
/* this is to get the actual fractions of each exposure level in each age group */
proc freq data=_tmp_;  tables &agegroup / out=_tmp1_ noprint;  by &exposure;  run;
data _tmp1_;  set _tmp1_;  pctexp=percent;  keep pctexp &agegroup &exposure;  run;
   
proc sort data=_tmp1_;  by &agegroup;  run;
proc sort data=_tmp_;  by &agegroup &exposure;  run;
proc sort data=mainfreq;  by &agegroup;  run;
   
data _tmp1_;  merge mainfreq _tmp1_;  by &agegroup;
   ...
   run;
proc sort data=_tmp1_;  by &agegroup &exposure;  run;
   
data _tmp_;  merge _tmp1_  _tmp_;  by &agegroup &exposure;  run;
...
proc sort data=_tmp_;  by &exposure &agegroup;  run;
proc means noprint data=_tmp_ n mean std median p25 p75;
  var &varlist;  weight wgt;  by &exposure;
   output  out=_stdm_  
      %if &nvar eq 1 %then %do;
	 mean=mean  std=std  n=n  median=median p25=p25 p75=p75;
	 %end;
   %else %do;
      mean=mean1-mean&nvar  std=std1-std&nvar   n=n1-n&nvar 
      median=median1-median&nvar p25=p251-p25&nvar p75=p751-p75&nvar  ;
      %end;
   run;
...
run;
proc sort data=descrips;  by &agegroup;  run;
data descrips;  merge descrips mainfreq;  by &agegroup;
...
data _stdm_;  merge _stdm_ stderrsq;  by &exposure;
...
%else %do;
   ...
   array p25s{*}  p251-p25&nvar;
   array p75s{*}  p751-p75&nvar;
   
  %do i=1 %to &nvar;

varname=trim(left("%scan(&varlist, &i, %str( ))"));
   ord=.;  ord=trim(left(&i));
   n=trim(left(ns{&i}));  mean=trim(left(mns{&i}));  std=trim(left(stds{&i}));  se=trim(left(ses{&i}));
median=trim(left(mdns{&i})); p25=trim(left(p25s{&i})); p75=trim(left(p75s{&i}));
   output;
  %end;
%end;
keep &exposure varname n mean std se median p25 p75 ord ;
   run;
   
proc sort data=_stdm_;  by ord varname &exposure;
data _stdm_;  set _stdm_;
   minexp=.;  maxexp=.;  minexp=&_minexp_;  maxexp=&_maxexp_;
   ...
   array p25s{&_minexp_ : &_maxexp_}  p25&_minexp_ - p25&_maxexp_;
   array p75s{&_minexp_ : &_maxexp_}  p75&_minexp_ - p75&_maxexp_;

   %do i=&_minexp_ %to &_maxexp_;  i=&i;
      if &exposure eq i then do;
	 ns{i}=n;  mns{i}=mean;  stds{i}=std;  ses{i}=se ;mdns{i}=median; p25s{i}=p25; p75s{i}=p75;
      end;
    %end;
...
var %do i=&_minexp_ %to &_maxexp_;  n&i mn&i std&i se&i mdn&i p25&i p75&i %end;
;
output  out=tab  mean=;
by ord varname;
run;
...
%do i=&_minexp_ %to &_maxexp_;
     n&i mn&i std&i se&i mdn&i p25&i p75&i    
%end;
;
